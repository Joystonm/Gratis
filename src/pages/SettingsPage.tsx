...
export function SettingsPage() {
  ...
  function handleSave() {
    ...
    toastSuccess(
      "Settings noted",
      "Set VITE_CLOUDINARY_CLOUD_NAME and VITE_CLOUDINARY_UPLOAD_PRESET in your .env file to configure Cloudinary",
    );
  }
  ...
  return (
    <div className="min-h-screen bg-canvas">
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
        <h1 className="text-3xl font-normal text-ink tracking-tight mb-2">
          Settings
        </h1>
        <p className="text-body mb-10">
          Configure Gratis to enable cloud-powered features.
        </p>
        ...
        <section className="bg-surface-card rounded-xl border border-hairline p-6 mb-6">
          ...
          <p className="text-sm text-body mb-4">
            Cloudinary enables cloud image storage, AI transformations, and CDN
            delivery. Uses only{" "}
          ...
          <div
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium mb-4 ${config.isConfigured ? "bg-success/10 text-success" : "bg-surface-strong text-muted"}`}
          >
            <div
              className={`w-2 h-2 rounded-full ${config.isConfigured ? "bg-success" : "bg-muted"}`}
            />
            {config.isConfigured
              ? "Cloudinary configured and active"
              : "Cloudinary not configured — using local-only mode"}
          </div>
          ...
          <div className="mt-4 p-3 rounded-lg bg-canvas border border-hairline text-xs text-muted space-y-1.5">
            ...
            <p>
              1. Create a <code className="font-mono">.env</code> file in the
              project root
            </p>
...
VITE_CLOUDINARY_UPLOAD_PRESET=your-unsigned-preset`}
            ...
            <p className="text-error/80">
              ⚠ Never add CLOUDINARY_API_SECRET to frontend code
            </p>
          </div>
        </section>
        ...
        <section className="bg-surface-card rounded-xl border border-hairline p-6">
          ...
          <p className="text-sm text-body mb-3">
            Gratis stores all projects and assets locally in your browser using
            IndexedDB. No data is sent to any server (except Cloudinary when
            configured).
          </p>
